...
async function updateRow(id, newData) {
  ...
  const sheets = getDoc();
  // Preserve existing source if not provided
  const existingRecord = rows[rowIndex];
  const source = newData.source !== undefined ? newData.source : (existingRecord.source || 'manual');
